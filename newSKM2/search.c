...
//	st_sen[DIST_SEN_R_FRONT].s_limit = 50;	//25
//	st_sen[DIST_SEN_L_FRONT].s_limit = 60;	//15
//	st_sen[DIST_SEN_R_SIDE].s_limit = 110;	//430
//	st_sen[DIST_SEN_L_SIDE].s_limit = 90;	//800

	switch(mypos.dir){
		case north:
		
			n_write = CONV_SEN2WALL( DIST_isWall_FRONT() == TRUE);	//センサが真（true)ならwall偽(false)ならNOWALL
			e_write = CONV_SEN2WALL( DIST_isWall_R_SIDE() == TRUE);
			w_write = CONV_SEN2WALL( DIST_isWall_L_SIDE() == TRUE);
			s_write = NOWALL;
//			printf("DIST_SEN_R_FRONT %5d \r\n",st_sen[DIST_SEN_R_FRONT].s_limit);
		...
		case east:
			...
			e_write = CONV_SEN2WALL( DIST_isWall_FRONT() == TRUE);
			s_write = CONV_SEN2WALL( DIST_isWall_R_SIDE() == TRUE);
			n_write = CONV_SEN2WALL( DIST_isWall_L_SIDE() == TRUE);
			w_write = NOWALL;
			break;
			
		case south:
		
			s_write = CONV_SEN2WALL( DIST_isWall_FRONT() == TRUE);
			w_write = CONV_SEN2WALL( DIST_isWall_R_SIDE() == TRUE);
			e_write = CONV_SEN2WALL( DIST_isWall_L_SIDE() == TRUE);
			n_write = NOWALL;
			break;
			
		case west:
		
			w_write = CONV_SEN2WALL( DIST_isWall_FRONT() == TRUE);
			n_write = CONV_SEN2WALL( DIST_isWall_R_SIDE() == TRUE);
			s_write = CONV_SEN2WALL( DIST_isWall_L_SIDE() == TRUE);
			e_write = NOWALL;
			break;
...
PUBLIC void fast_run(int x, int y,float f_speed)
{
	...
	MOVE_BACK_DIST = 0.3;
	max_speed = MOT_setTrgtSpeed(f_speed);
